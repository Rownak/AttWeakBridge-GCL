...
class GPTPromptTuningMixin:
    ...
    def _cat_learned_embedding_to_input(self, input_ids) -> torch.Tensor:
        ...
        # Handling Error 79 inputs_embeds = torch.cat([learned_embeds, inputs_embeds], dim=1)
        # RuntimeError: Tensors must have same number of dimensions: got 3 and 4
        #learned_embeds = self.soft_prompt.weight.unsqueeze(0).expand(inputs_embeds.size(0), -1, -1)

        #print(f"learned_embeds shape: {learned_embeds.shape}")
        #print(f"inputs_embeds shape before cat: {inputs_embeds.shape}")

        inputs_embeds = torch.cat([learned_embeds, inputs_embeds], dim=1)

        #print(f"inputs_embeds shape after cat: {inputs_embeds.shape}")
